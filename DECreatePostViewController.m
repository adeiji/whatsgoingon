...
- (IBAction)displayPreview:(id)sender {
    ...
    if ([_createPostViewTwo page2ValidateTextFields])
    {
        ...
        id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
        [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"MakePost2"
                                                              action:@"Button Click"
                                                               label:@"Preview and Post Button Tapped"
                                                               value:nil] build]];
...
- (IBAction)gotoNextScreen:(id)sender {
    ...
    {
        id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
        [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"MakePost"
                                                              action:@"Button Click"
                                                               label:@"Next Button Tapped"
                                                               value:nil] build]];

        if ([DEPostManager getDurationOfEvent:_post] > [NSNumber numberWithInt:0]) {
            [self displayNextScreen];
        }
        else {
            [_createPostViewOne displayErrorOnEndTime];
        }
    }
}
...
- (IBAction)continueGoingBack:(id)sender {
    ...
    {
        id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
        [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"CancelPost"
                                                              action:@"ButtonClick"
                                                               label:@"Return to Post Screen Tapped"
                                                               value:nil] build]];
        ...
        [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"CancelPost"
                                                              action:@"ButtonClick"
                                                               label:@"Exit and lose post info Tapped"
                                                               value:nil] build]];
