...
- (void) updatePost {
    ...
    [[finishedPostView lblParagraphFour] setText:@"Need more posting options?  Full featured posting available on HappSnap.com"];
...
- (IBAction) savePost : (id)sender {
    ...
    id<GAITracker> tracker = [[GAI sharedInstance] defaultTracker];
    [tracker send:[[GAIDictionaryBuilder createEventWithCategory:@"PreviewPost"
                                                          action:@"ButtonClick"
                                                           label:@"Post Away Button Tapped"
                                                           value:nil] build]];
    ...
    if (postSaved)
    {
        ...
        {
            finishedPostView.lblParagraphOne.text = @"PLEASE NOTE: Your event will be available for viewing 3 days from its start time.";
...
- (void) actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    {
        NSString *urlString = [NSString stringWithFormat:APPLE_MAPS_APP_URL, [_post.address stringByReplacingOccurrencesOfString:@" " withString:@"+"], _post.location.longitude, _post.location.latitude ];
